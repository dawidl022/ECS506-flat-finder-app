...
class ZooplaClient(APIClient):
    ...
    def fetchListing(listing_id: int) -> ExternalAccommodationListing | None:
        ...
        livingConditions:list[str] = [response['listing'][0]['furnished_state']]
    ...
    @staticmethod
    def searchListing(area: str, radius: float, order_by: str, page_number: int, page_size: int, maximum_price: int =0) -> list[ExternalAccommodationListing]:
        url = "https://zoopla.p.rapidapi.com/properties/list"
        ...
        for x in response['listing']:
            ...
            src: str = ZooplaClient.name
            rating: float = 0.0  # zoopla does not have ratings
            out.append(ExternalAccommodationListing(img, type, numrooms, livingConditions, amen, listurl, src, rating))
